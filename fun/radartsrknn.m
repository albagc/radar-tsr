function [Ximp, cwpred, rwpred, cltagE, modeloutput] = radartsrknn(X, A, ...
...
% - - - - - - - - - - - - - - - - - - - - -
[mbloop, ~, ~, pca0] = pcambloop(Xnai_wocw(~m0,:), A, Mmd(~m0,:), Mall(~m0,:), find(~m0), alpha, ...
...
%% Step 4: NA-imputation, detection and TSR for PCA-MB without any rowwise nor cellwise outliers
...
%% Step 5: Final imputation, projection and detection
